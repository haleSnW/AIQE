...
3. [适用对象：不同产品形态怎么用 AIQE](#3-适用对象不同产品形态怎么用aiqe)
4. [快速开始：mock 后端 30 秒出报告](#4-快速开始mock-后端-30-秒出报告)
5. [框架结构](#5-框架结构)
6. [方法论文档：分层、流程、协作、触发](#6-方法论文档分层流程协作触发)
7. [与开发 harness 同步使用：pytest、CI、基线、换模型流程](#7-与开发harness同步使用pytestci基线换模型流程)
8. [示例集：ollama 接入与回归对比](#8-示例集ollama-接入与回归对比)
9. [测试触发策略：四档 × 自动/手动](#9-测试触发策略四档-自动手动)
10. [模板集与配套 skill](#10-模板集与配套skill)
11. [开源协议](#11-开源协议)
12. [捐赠支持（最后一项）](#12-捐赠支持)
...
| 模型（大模型/端侧模型） | 跑分与换版回归：全量用例集对比基线，五步流程（见第 7 节）定去留 |
...
配合使用方法（pytest 共存 / CI 接入 / 基线 SOP / 换模型五步）见第 7 节。

## 7. 与开发 harness 同步使用：pytest、CI、基线、换模型流程
...
AIQE 是完全开源、免费使用的项目。如果你觉得它有用，可以考虑支持一下
开发与维护。每一笔支持都会记录去向并公示在仓库 release 说明中，
用途仅限三项：

| 用途 | 说明 |
|------|------|
| 真实推理验证 | 用例集在真机（Apple Silicon / MLX / Ollama）上跑全量回归的算力成本 |
| 用例集扩充 | 新场景用例的构造与人工校准 |
| 维护与文档 | 框架维护、CI、文档与多语言翻译 |
